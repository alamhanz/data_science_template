...
def gcs2bq(config, disposition='WRITE_TRUNCATE',file_format=bigquery.SourceFormat.CSV):
    ...
    load_job = bq_client.load_table_from_uri(
            uri, bq_table_id, job_config=job_config)  # Make an API request.
...
def table2bq(df,config, disposition='WRITE_TRUNCATE',file_format=bigquery.SourceFormat.CSV):
    # WRITE_APPEND
    project_id = config['project_name']
    dataset_id = config['dataset_name']
    table_name = config['table_name']
    ...
    load_job = bq_client.load_table_from_dataframe(
            df, bq_table_id, job_config=job_config)  # Make an API request.
