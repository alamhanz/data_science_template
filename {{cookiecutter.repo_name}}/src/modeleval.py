from sklearn.metrics import mean_squared_error, make_scorer,r2_score, classification_report, confusion_matrix, mean_squared_log_error
...
def regression_eval(ytrue,ypred,thrs=3):
    ...
    summary_eval['MAPE_undrpred'] = mape_score(underpred_pair[:,0],underpred_pair[:,1])
    
    summary_eval['over_est_'+str(thrs)+'_pct']=(ypred-ytrue>thrs).mean()*100
    summary_eval['under_est_'+str(thrs)+'_pct']=(ytrue-ypred>thrs).mean()*100
