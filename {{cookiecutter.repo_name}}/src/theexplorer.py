...
def eval_features(df_input, col_label="y", col_features_selected=["1", "2"]):
    ...
    for c in col_features_selected:
        ...
        while steps + range_step < 100:
            try:
                bins.append(
                    (
                        np.percentile(df_features1, steps),
                        np.percentile(df_features1, steps + range_step),
                    )
                )
        ...
        for bin0 in bins:
            df1_probs.append(
                len(df_1[(df_1[c] >= bin0[0]) & (df_1[c] < bin0[1])]) / float(len(df_1))
            )
            df2_probs.append(
                len(df_2[(df_2[c] >= bin0[0]) & (df_2[c] < bin0[1])]) / float(len(df_2))
            )

        df1_probs.append(len(df_1[(df_1[c] >= bins[-1][1])]) / float(len(df_1)))
        df2_probs.append(len(df_2[(df_2[c] >= bins[-1][1])]) / float(len(df_2)))
        ...
        df_prob_dist["psi_calc2"] = (
            (np.log(df_prob_dist["prob1"] / df_prob_dist["prob2"]))
            .fillna(0)
            .replace(np.inf, 10)
        )
    ...
    gini_per_features = pd.DataFrame.from_dict(d, orient="index").rename(
        columns={0: "gini"}
    )
    gini_per_features["gini_abs"] = np.abs(gini_per_features["gini"])

    psi_per_features = pd.DataFrame.from_dict(p, orient="index").rename(
        columns={0: "psi_score"}
    )
